...
/**
 * Shared Tailwind theme preset for the Hamza Store monorepo.
 *
 * Holds the design tokens used by BOTH apps (admin brand/accent ramps + the
 * fixed storefront "store" palette + fonts/radii/shadows/animations). Each app's
 * tailwind.config consumes this via `presets: [sharedPreset]` and supplies its
 * own `content` globs. Semantic colour tokens are CSS variables that flip in dark
 * mode (see each app's globals.css).
 */
